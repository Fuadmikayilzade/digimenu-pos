...
export default function KitchenCard({ ticket, tableLabel, onUpdate, onReady, canControl }) {
    ...
    <div className="kitchen-card">
      <div className="kc-header">
        <span className="kc-table">
          Masa {tableLabel || ticket.table}
          {ticket.staffName ? <span style={{ fontSize: 10, color: 'var(--gray2)', fontWeight: 500, marginLeft: 6 }}>· {ticket.staffName}</span> : null}
        </span>
      ...
      <div className="kc-items">
        {ticket.items.map((it, i) => (
          <div key={i} className="kc-item">
            ...
            <div className={`kc-done${it.done ? ' checked' : ''}${!canControl ? ' readonly' : ''}`} onClick={() => toggleDone(i)}>
              {it.done ? '✓' : ''}
            </div>
          </div>
        ))}
      ...
      <div className="kc-footer">
        ...
        {canControl ? (
          ...
          // Ofisiant/menecer paneli görə bilir, amma "Hazır" yalnız
          // mətbəx işçisinin (və sahibkar/adminin) səlahiyyətindədir:
          <span className="kc-ready-readonly">👀 Yalnız baxış</span>
        )}
      </div>
    </div>
  )
