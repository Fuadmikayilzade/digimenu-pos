...
export default function DashboardPage() {
  // ⚠️ ƏVVƆLKİ KOD öz AYRICA `branchFilter` state-inə malik idi —
  // yuxarıdaki (Topbar) filial seçimi ilə heç bir bağlılığı yox idi.
  // Sahibkar Topbar-dan filial seçəndə bu səhifə hələ də "bütün
  // filiallar"ı göstərməyə davam edirdi. İndi HAMISI eyni qlobal
  // `activeBranchId`-ə tabedir:
  const { user, paidOrders, todaySales, openTables, ticketCount, orders, topItems, activeBranchId, branches } = useApp()
  ...
  const loadRemote = async () => {
    ...
    setUseRemote(true)
    try {
      const data = await api.reports.daily(selectedDate, activeBranchId)
      setRemoteData(data)
    } catch {
      setUseRemote(false)
      setRemoteData(null)
  ...
  useEffect(() => {
    if (!user) return
    api.reports.daily(toISODate(), activeBranchId)
      .then(data => { setRemoteData(data); setUseRemote(true) })
      .catch(() => {})
  }, [user, activeBranchId])

  // ⚠️ ƏVVƆLKİ KOD YALNIZ 'admin' rolunu qəbul edirdi — 'owner' (sahibkar)
  // VƏ 'manager' (menecer) tamamilə İSTİSNA edilirdi. Ortaq `can()`
  // funksiyası (constants.js) ilə düzəldildi:
      ...
      <div className="dash">
        ...
        <div className="date-toolbar" style={{ marginBottom: 16 }}>
          ...
          <input type="date" className="date-input" value={selectedDate}
            ...
            max={toISODate()} />
          <span style={{ fontSize: 11, color: 'var(--gray2)', padding: '6px 10px', borderRadius: 8, background: 'var(--bg3)' }}>
            🏢 {activeBranchId ? (branches.find(b => b.id === activeBranchId)?.name || 'Filial') : 'Əsas filial'}
          </span>
